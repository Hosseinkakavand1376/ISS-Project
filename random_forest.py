...
def main(args):
    ...
    file_paths = [f"{args.dataset_path}/part-0000{i}-363d1ba3-8ab5-4f96-bc25-4d5862db7cb9-c000.csv" for i in
                  range(args.num_files)]
    ...
    X_train_pca, X_test_pca, y_train_pca, y_test_pca = train_test_split(X_pca, y, test_size=args.test_size,
                                                                        random_state=42)
    ...
    # Creating the optimized RandomForestClassifier with best parameters
    rf_optimized = RandomForestClassifier(**best_params, random_state=42)
    rf_optimized.fit(X_train_pca, y_train_pca)

    # Predicting with optimized RandomForestClassifier
    Y_pred_rf_optimized = rf_optimized.predict(X_test_pca)
    Y_pred_rf_optimized_proba = rf_optimized.predict_proba(X_test_pca)

    # Classification Report for optimized model
    report_optimized = classification_report(y_test_pca, Y_pred_rf_optimized)
    print("Optimized Classification Report:")
    print(report_optimized)

    # Precision-Recall Curve for each class
    y_test_pca = y_test.argmax(axis=1)  # Convert one-hot encoded labels to single labels

    for i, class_label in enumerate(onehot_encoder.categories_[0]):
        PrecisionRecallDisplay.from_predictions(y_test_pca == i, Y_pred_rf_optimized_proba[:, i],
                                                name=class_label).plot()
